...
# 1. Input: Đọc file JSON mà Job 1 vừa tạo ra
INPUT_PATH = f"{HDFS_PREFIX}/data/preprocessed_data.json"

# 2. Output HDFS: Lưu trữ dữ liệu sạch cuối cùng dạng Parquet (Tối ưu lưu trữ lâu dài)
OUTPUT_PATH_HDFS = f"{HDFS_PREFIX}/data/cleaned_recruitment_data"

# 3. Output DB: Data Warehouse (Postgres)
DB_CONFIG = {
    "url": "jdbc:postgresql://postgres-service.postgres.svc.cluster.local:5432/airflow_dw",
    "user": "airflow",
    "password": "airflow",
    "driver": "org.postgresql.Driver",
    "table": "public.recruitment_fact"
}
...
print(f"--- [JOB 2] Đọc dữ liệu JSON từ: {INPUT_PATH}")
try:
    # QUAN TRỌNG: Đọc file JSON (Output của Job 1)
...
print(f"Số bản ghi đầu vào: {df.count()}")
...
# 1. Ghi xuống HDFS (Parquet)
print(f"--- [JOB 2] Ghi Parquet xuống HDFS: {OUTPUT_PATH_HDFS}")
df_cleaned.write.mode("overwrite").parquet(OUTPUT_PATH_HDFS)

# 2. Ghi vào Postgres
print("--- [JOB 2] Ghi vào Data Warehouse (Postgres)...")
try:
    (
        df_cleaned.write
        .format("jdbc")
