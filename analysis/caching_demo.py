...
# Mục tiêu: đọc cleaned_recruitment_data.json -> chuẩn hóa schema -> materialize -> thử performance
# Ghi chú: mọi phần quan trọng đều có comment giải thích (VN)
...
# 1) SPARK SETUP
# ------------------------------
# Khởi tạo SparkSession với vài cấu hình hữu ích cho local testing.
# - spark.sql.shuffle.partitions: giảm số partition cho job nhỏ, giúp benchmark nhanh hơn trên máy local.
# - active faulthandler config để dễ debug lỗi Python UDF.
...
# 2) INPUT / TEMP PATHS
# ------------------------------
# input_file: file JSON lines đầu vào (đã clean)
# _temp_parquet: thư mục tạm để write parquet (materialize) nhằm loại Python UDF khỏi logical plan
...
# ------------------------------
# 3) ĐỌC JSON (LINE-BY-LINE)
# ------------------------------
# Đọc file JSON lines. Nếu file corrupt sẽ raise, bắt exception để debug.
# xóa file đuôi .crc tránh lỗi checksum để chạy
...
# ------------------------------
# 4) CHUẨN HÓA TÊN CỘT (HANDLE DYNAMIC SCHEMA)
# ------------------------------
# Vì nguồn JSON không đồng nhất, ta rename các cột tương đương về cùng tên chuẩn dùng trong phân tích.
# Mục đích: tránh lỗi khi groupBy / agg vì các file có tên trường khác nhau.
print("[3] Current columns:", current_cols)
...
print(f"[4] Standardized dataset rows: {df.count()}. Columns now: {df.columns}")

# ------------------------------
# 5) MATERIALIZE: write -> read parquet
# ------------------------------
# Mục đích: loại bỏ Python UDF (nếu có) khỏi logical plan, cho Spark đọc native parquet
# và giúp cache/persist hoạt động hiệu quả hơn (khi đọc từ parquet Spark có toàn bộ metadata)
...
print(f"[6] Materialized DataFrame loaded. Count = {final_mat.count()}")

# ------------------------------
# 6) DEMO WITHOUT CACHING (BASELINE)
# ------------------------------
...
# .persist(StorageLevel.MEMORY_AND_DISK) = an toàn hơn cho dataset lớn
...
# Không phải chạy lại DAG (Direct Acyclic Graph).

# Giảm số stage, giảm shuffle.

# Tăng throughput của cluster.

# ✔ 3. Resource allocation

# Khi cache, Spark dùng RAM nhiều hơn
...
# Dùng Spark UI (http://localhost:4040
# ) để xem:
...
# Cache đã được đánh dấu (cached RDD/DataFrame).